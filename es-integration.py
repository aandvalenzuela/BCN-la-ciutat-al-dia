...
# Consolidate last's week data. Comment out the following 2 lines to upload the entire dataset.
...
try:
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        payload = row.to_dict()
        unique_id = sha1(f"{payload['Data_Indicador']}/{payload['Nom_Indicador']}/{payload['Nom_Variable']}".encode("utf-8")).hexdigest()
